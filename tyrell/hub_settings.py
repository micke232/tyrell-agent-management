...
def local_report(codex='codex'):
    ...
    for key, executable in (('codex', shutil.which(codex)), ('copilot', copilot_executable())):
        ...
        if executable:
            ...
            args = ['app-server', '--help'] if key == 'codex' else ['--help']
            code, help_text = command_info([executable, *args])
            if key == 'codex':
                info['compatible'] = code == 0 and 'proxy' in help_text and 'daemon' in help_text
            else:
                async def protocol():
                    probe = CopilotProbe()
                    try:
...
def settings_text(data, directory):
    ...
    for key, name in (('codex', 'Codex'), ('copilot', 'GitHub Copilot')):
        info = values.get(key, {})
        label = LABELS.get(info.get('status'), ('Offline', 'warning'))[0]
        lines.append('**' + name + '** · ' + label + ' · ' + str(len(info.get('models', []))) + ' models')
        installed = data.get('installation', {}).get('providers', {}).get(key)
        if installed:
            lines.append('CLI: ' + (installed.get('version') or 'Not installed') + ' · ' + ('Compatible' if installed.get('compatible') else 'Setup needed'))
    lines += ['', 'One connected provider is enough. CLI installation and account access are separate.',
              '`[1]` Codex installation & sign-in', '`[2]` Copilot installation & sign-in',
...
def provider_guide(provider, host='https://github.com'):
    if provider == 'codex':
        return ('# Connect Codex\n\n## 1 · Install its CLI\n'
                ...
                '## 2 · Sign in\n`tyrell login codex`\n\n'
                '## 3 · Start its local server\n`tyrell start-provider codex`\n'
                'This integration requires a CLI with app-server proxy and daemon support.\n'
