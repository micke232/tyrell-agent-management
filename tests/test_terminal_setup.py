...
from tyrell.terminal_setup import setup_terminal, terminal_info
...
class TerminalSetupTests(unittest.TestCase):
    def run_setup(self, info, answer='2', result=0):
        ...
        with patch('tyrell.terminal_setup.terminal_info', return_value=info), patch('builtins.input', return_value=answer), patch('tyrell.terminal_setup.subprocess.run', return_value=Mock(returncode=result)) as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_install_only_after_explicit_choice_and_check_result(self):
        ...
        with tempfile.TemporaryDirectory() as d, patch('tyrell.terminal_setup.terminal_info',side_effect=[missing,installed]), patch('builtins.input',return_value='3'), patch('tyrell.terminal_setup.subprocess.run',return_value=Mock(returncode=0)) as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_noninteractive_setup_never_installs_or_prompts(self):
        with tempfile.TemporaryDirectory() as d, patch('builtins.input') as read, patch('tyrell.terminal_setup.subprocess.run') as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_install_failure_is_not_saved_as_success(self):
        with tempfile.TemporaryDirectory() as d, patch('tyrell.terminal_setup.terminal_info',return_value={'ghostty':None,'current':'Apple_Terminal','installer':'/brew'}), patch('builtins.input',return_value='3'), patch('tyrell.terminal_setup.subprocess.run',return_value=Mock(returncode=1)), contextlib.redirect_stdout(io.StringIO()):
