...
class HomebrewPackageTests(unittest.TestCase):
    def fixture(self, root):
        path = root/'tyrell_agent_management-0.2.0a2-py3-none-any.whl'
        with zipfile.ZipFile(path,'w') as archive:
            archive.writestr('tyrell_agent_management-0.2.0a2.dist-info/METADATA','Name: tyrell-agent-management\nVersion: 0.2.0a2\n')
        return path

    def test_archive_formula_checksum_and_url_match(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);wheel=self.fixture(root)
            archive,formula=build(wheel,'example/tyrell',root/'out')
    ...
    def test_mismatched_wheel_metadata_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises((KeyError,ValueError)):
                build(renamed,'example/tyrell',root/'out')
