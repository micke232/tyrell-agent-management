...
def build(wheel, repository, output):
    ...
    match = re.fullmatch(r'agent_hub_management-([0-9][A-Za-z0-9.+]*)-py3-none-any.whl', wheel.name)
    if not match:
        raise ValueError('Choose a Tyrell Agent Management universal wheel')
    version = match[1]
    with zipfile.ZipFile(wheel) as archive:
        metadata = archive.read('agent_hub_management-' + version + '.dist-info/METADATA').decode()
        if 'Name: agent-hub-management\n' not in metadata or 'Version: ' + version + '\n' not in metadata:
            raise ValueError('Wheel metadata does not match its filename')
    output.mkdir(parents=True, exist_ok=True)
    filename = 'agent-hub-' + version + '-homebrew.tar.gz'
    ...
    with archive_path.open('wb') as raw, gzip.GzipFile(fileobj=raw, mode='wb', filename='', mtime=0) as compressed, tarfile.open(fileobj=compressed, mode='w') as tar:
        for name, data in ((wheel.name, wheel.read_bytes()), ('INSTALL.md', (ROOT/'docs/install.md').read_bytes())):
            info = tarfile.TarInfo('agent-hub-' + version + '/' + name)
            info.size, info.mode, info.mtime = len(data), 0o644, 0
            tar.addfile(info, io.BytesIO(data))
    checksum = hashlib.sha256(archive_path.read_bytes()).hexdigest()
    homepage = 'https://github.com/' + repository
    url = homepage + '/releases/download/v' + version + '/' + filename
    text = (ROOT/'packaging/homebrew/agent-hub.rb.in').read_text()
    ...
    (tap/'Formula/agent-hub.rb').write_text(text)
    (tap/'HOMEBREW.md').write_text('# Install Tyrell Agent Management with Homebrew\n\nKeep Formula/agent-hub.rb in the same repository as the app.\n\n```sh\nbrew tap ' + repository + ' https://github.com/' + repository + '.git\nbrew install ' + repository + '/agent-hub\ntyrell\n```\n')
    (output/'SHA256SUMS').write_text(checksum + '  ' + filename + '\n')
    return archive_path, tap/'Formula/agent-hub.rb'
